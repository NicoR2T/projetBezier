...
def zoom(echelle, event):
    global rayon,X,Y
    canvas.scale(ALL,centreX,centreY,echelle,echelle)
    rayon*=echelle
    for i in range(0,degre):
        if X[i]>centreX:
            X[i]=(X[i]-centreX)*echelle+centreX
        else:
            X[i]=centreX-((centreX-X[i])*echelle)
        if Y[i]>centreY:
            Y[i]=(Y[i]-centreY)*echelle+centreY
        else:
            Y[i]=centreY-(centreY-Y[i])*echelle
    if degre >=1:
        for elt in canvas.find_withtag("courbe"):
            canvas.delete(elt)
        if degre >1:
